...
let rec exec stmt (locEnv : locEnv) (gloEnv : gloEnv) (store : store) : store = 
    ...
    | For(e1,e2,e3,body) ->
      ...
      let rec loop store1 = 
        ...
        if v<>0 then 
                     let (_,store4) = eval e3 locEnv gloEnv (exec body locEnv gloEnv store2)
                     loop store4
                else store2
      loop store1
     | Switch(e1, caseList) -> 
...
and eval e locEnv gloEnv store : int * store = 
    ...
    | PreInc acc     -> let (loc, store1) = access acc locEnv gloEnv store
                        let tmpVal = (getSto store1 loc)  + 1
                        // (printf "%d " tmpVal; tmpVal)
                        let sotre2 = setSto store1 loc tmpVal
                        (tmpVal,sotre2)
